...
export class Finder {
	...
	constructor(options = {}) {
		...
		if (!paths.length) {
			...
			if (pathEnv.length) paths = pathEnv.split(";");
		}

		if (!extensions.length) {
			const pathExt = process.env.PATHEXT ?? "";
			extensions = pathExt.length ? pathExt.split(Finder.isWindows ? ";" : delimiter) : [".exe", ".cmd", ".bat", ".com"];
		}

		this.extensions = extensions.map(extension => extension.toLowerCase());
		this.paths = paths.map(directory => directory.replace(/^"+|"+$/g, ""));
	}
	 ...
	 * @param {string} file The path of the file to be checked.
	 * @returns {boolean} Value indicating whether the specified file is executable.
	 */
